...
document.addEventListener('DOMContentLoaded', () => {
    ...
    function displayPackageDetails(pkg) {
        ...
        if (pkg.tipo === 'DATA_FIXA' && pkg.data_ida && pkg.data_volta) {
            ...
            secaoDatas = `
                <div class="d-flex align-items-center mb-3"><i class="bi bi-calendar-event fs-4 me-2"></i><span><strong>Datas flexíveis</strong><br><small>Disponível o ano todo.</small></span></div>
            `;
        }

        // --- CORREÇÃO NOS LINKS DO BREADCRUMB ABAIXO ---
    ...
    enviarReservaBtn.addEventListener('click', async () => {
        ...
        try {
            const response = await fetch('http://127.0.0.1:8000/api/solicitar-reserva/', {
                ...
                body: JSON.stringify({pacoteId: pacoteId, nome: nome, email: email, telefone: telefone})
            });
        ...
        const numeroAgencia = '5511999999999';
